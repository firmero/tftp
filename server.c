...
get_server_socket(char *portstr)
{
	...
	Source:
			www.stackoverflow.com/questions/34837978/problems-with-ipv6-connect-in-c
	Dual stack:
...
main(int argc, char **argv)
{
	...
	int n = recvfrom(fd, buff, BUF_LEN, 0, (struct sockaddr *)&ca, &st_sz);
	if (n == -1) {
	    warn("recvfrom ");
	    continue;
	}
	...
	case OP_RRQ:;
	    ...
	    node_t *node_p = create_node(n - OPCODE_SIZE, buff + OPCODE_SIZE, ca);
	    append_node(node_p);

	    if (pthread_create(&node_p->tid, &a, rrq_serve, node_p) != 0) {
		warn("pthread_create");
	...
	case OP_WRQ:
	    ...
	    node_p = create_node(n - OPCODE_SIZE, buff + OPCODE_SIZE, ca);
	    append_node(node_p);

	    if (pthread_create(&node_p->tid, &a, wrq_serve, node_p) != 0) {
		warn("pthread_create");
		// todo release resources?
	    }
