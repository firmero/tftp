...
int
get_filename_mode(const char *buff, const size_t sz, char **filename, char **mode)
{
...
rrq_serve(void *p_node)
{
	...
	send_err(fd, node_p->saddr_st, EE_NOTDEFINED, "Server support only octet mode transmission.");
	remove_node(node_p);
	return NULL;
    }
